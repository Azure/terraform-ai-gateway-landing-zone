# =============================================================================
# Azure OpenAI API submodule
# Mirrors bicep/infra/modules/apim/inference-api.bicep with
# inferenceAPIType='AzureOpenAI' and the azure-openai wiring from apim.bicep.
#
...
#     OpenAPI spec (AIFoundryOpenAI.json), which provides the full Azure
#     OpenAI surface: /deployments, /deployments/{id}, /deployments/{id}/{
#       completions, embeddings, chat/completions, audio/transcriptions,
#       audio/translations, audio/speech, images/generations, ...}
#   - Microsoft.ApiManagement/service/apis/diagnostics (azuremonitor)
#     including largeLanguageModel logs (azapi)
...
# Bicep parity: openAIDeploymentOperationPolicy /
# openAIDeploymentByNameOperationPolicy (both reuse the universal-llm policy
# XMLs). Gated on llm-backend availability since the policies reference the
# dynamic `get-available-models` fragment.
# -----------------------------------------------------------------------------
...
# property, so we PATCH it here. Required for `<emit-metric>` /
# `<llm-emit-token-metric>` samples to reach App Insights customMetrics.
