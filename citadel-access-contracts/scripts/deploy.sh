...
#   ./scripts/deploy.sh --destroy                # Remove the use-case resources
# =============================================================================
...
# --- Terraform action ---
if [[ -n "$DESTROY" ]]; then
  warn "DESTROYING the onboarded use-case resources..."
...
try:
    ...
    for code, p in data.items():
        print(f\"  {code}: {p['product_id']}\")
except Exception:
    print('  (run terraform output products to inspect)')
" 2>/dev/null || true
fi
